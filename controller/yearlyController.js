...
module.exports = function (req, res, next) {
    ...
    if (cache["date"] == new Date().getFullYear()) {
    ...
    const result = fetch(url)
        ...
        .then((text) => {
            ...
            const jsonObject = { title: horoscopeList[horoscopeName][0].turkishToUpper() + horoscopeList[horoscopeName].slice(1) + " Burcu Yıllık Yorum", content: htmlContent[1].textContent.replace(/&rsquo;/g, "'").replace(/\n/g, "") };
            const response = res.json(jsonObject);
            cache["object"] = jsonObject
            cache["date"] = new Date().getFullYear()
            return response;
        })
